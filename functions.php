...
if ( ! defined( 'UMBLAETTERER_VERSION' ) ) {
	...
	define( 'UMBLAETTERER_VERSION', '1.0.0' );
}
...
function umblaetterer_setup() {
	/*
		...
		* If you're building a theme based on Umblätterer, use a find and replace
		* to change 'umblaetterer' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'umblaetterer' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'umblaetterer_custom_background_args',
			array(
...
function umblaetterer_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'umblaetterer' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'umblaetterer' ),
...
function umblaetterer_scripts() {
	...
	wp_enqueue_script( 'umblaetterer-navigation', get_template_directory_uri() . '/js/navigation.js', array(), UMBLAETTERER_VERSION, true );
