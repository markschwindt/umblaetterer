...
- **The wordmark writes itself on load.** The fourteen paths in the SVG are the fourteen letters, already in left-to-right order, and `umblaetterer_wordmark()` numbers each one with a `--i` custom property — they sit in two Illustrator groups, so a CSS `:nth-child` stagger would restart counting at the *U*. Each letter is then revealed by a `clip-path` that sweeps down across it, with its leading edge running about forty percent lower on the left than on the right: roughly a twenty-five-degree slant, which is a broad nib's angle. A level wipe reads as a shutter; a slanted one reads as a pen. 240ms a letter, 32ms apart, the whole word in about two-thirds of a second, and nothing at all under `prefers-reduced-motion`.
...
- **The icon carries its own `prefers-color-scheme` block**, inside the SVG's `<style>`: navy on a light browser chrome, ivory on a dark one, because the navy all but vanishes against a dark tab bar at 16px. The query is answered by the *browser's* colour scheme, not the page's, so it is independent of the Tagausgabe/Nachtausgabe switch. Two caveats: the `mask-icon` Safari uses for pinned tabs is tinted by its own `color` attribute and does not follow this, and browsers cache favicons hard — the URL is versioned off `UMBLAETTERER_VERSION`, so bump it when the file changes.
...
- **Nothing in the colophon may break across a column**, and the columns still have to fill. Those two pull against each other, and the arithmetic is worth knowing: with `break-inside: avoid`, the column height is at least the tallest block, so the columns actually used come out as *total content ÷ tallest block*. A 54-item page list stood 1,681px tall and pinned the footer to three columns of four.
  Two rules fix it, both keyed off the list's own length so they keep working for whatever gets added later. `ul:has(li:nth-child(20))` folds a long list into sub-columns inside its block; `:has(li:nth-child(40))` takes a block too long for that out of the flow entirely with `column-span: all`, giving it the whole measure across the top. Neither breaks a block — the spanning one is whole, just wide. Footer went 1,716px → 1,157px and 3 columns → 4. Browsers without `:has()` simply fall back to three columns.
- **The badges band carries no borders at all.** The four badges stand side by side with nothing ruled between them; the line that divides the band from the colophon is drawn from the colophon's `border-bottom`, not from the band. If rules are ever wanted back between the items, note that a plain `border-left` will not do — flex wraps, and a breakpoint cannot know where. The technique that does work is a clipped box-shadow: each item casts a hairline up and to its left, the container clips what falls off the edges, and the items must then carry no `max-width` or the leftmost one stops starting at the container edge.
