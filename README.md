...
- **The badges band draws its inner rules with clipped box-shadows, not borders.** Each item casts a hairline up and to its left and the container clips what falls off the edges, so the separators land correctly however the flex line wraps — four across, two and two, or stacked. A breakpoint cannot know where the wrap falls; this does not have to. It also means the items must not carry a `max-width`, or the leftmost one stops starting at the container edge and its rule escapes the clip. The band's own opening rule is a plain `border-top`, because the first row's shadows fall outside the padding box and are clipped away.
